...
export class VisitService {
    ...
    getVisit (root: string, encounterId: string): Visit {
        let visits: Visit[] = JSON.parse(this._store.getValue(this._storedDataName + '.data')).map((req: any) => Visit.parseObj(req));
        let url = '';
        ...
        let index = visits.findIndex(f => f.encounterId === encounterId && f.root && root);
