...
export class UpdaterService implements OnInit {
    ...
    checkUpdateAgentInstallation(): void {
        this._http.get<any>(this._url.parse('updateAgentInstalled')).pipe(
    ...
    getUpdateInfo(): void {
        if (this.isUpdateAgentInstalled) {
            this._http.get<any>(this._url.parse('updateDWH')).pipe(
                catchError(err => { return this._http.handleError(err); }))
                .subscribe(event => {
                    if (event._body) {
    ...
    getUpdateLog(): void {
        if (this.isUpdateAgentInstalled) {
            this._http.get<any>(this._url.parse('getUpdateLog')).pipe(
                catchError(err => { return this._http.handleError(err); }))
                .subscribe(event => {
                    if (event._body)
    ...
    checkUpdateSuccess(): void {
        if (this.isUpdateAgentInstalled) {
            this._http.get<any>(this._url.parse('checkUpdateSuccess')).pipe(
                catchError(err => { return this._http.handleError(err); }))
                .subscribe(event => {
                    if (event._body)
