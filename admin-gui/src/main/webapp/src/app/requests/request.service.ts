...
export class RequestService {
     ...
     */
    getRequest(requestId: number, etag: string, mapping: boolean): Observable<any> {
        let url = '';
        if (mapping === true) {
            url = 'request';
        } else {
    ...
    updateStatus(requestId: number, status: RequestStatus, autoSubmit?: boolean): RequestStatus {
        let currentRoute = this._router.url;
        let setStatusPost = this._http.post(this._urls.parse('setRequestStatus', {
            requestId: requestId,
        ...
        if (autoSubmit) {
            this._http.post(this._urls.parse('setRequestAutoSubmit', {
                requestId: requestId,
                submit: autoSubmit
            }), {}).pipe(
                catchError(err => {
